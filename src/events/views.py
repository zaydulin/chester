    ...
    def get_context_data(self, **kwargs):
        ...
        for league_name, events_in_league in groupby(events_page, key=lambda event: event.section):
            ...
            for event in events_list:
                ...
                event_bookmarked = Bookmarks.objects.filter(user=user, content_type=event_content_type,
                                                            object_id=event.id).exists()
                league_bookmarked = Bookmarks.objects.filter(user=user, content_type=league_content_type,
                                                             object_id=event.section.id).exists() if event.section else False
...
class EventsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        no_comands = event.rubrics.sortable not in [1, ]
        context['no_comands'] = no_comands

        football = event.rubrics.sortable in [1, ]
        context['football'] = football

        hockey = event.rubrics.sortable in [2, ]
        context['hockey'] = hockey

        tennis = event.rubrics.sortable in [3, ]
        ...
        if self.request.user.is_authenticated:
            ...
            home_team_bookmark = Bookmarks.objects.filter(
                ...
                object_id=home_team.id
            ).first()
            away_team_bookmark = Bookmarks.objects.filter(
                user=user,
                content_type=ContentType.objects.get_for_model(Team),
                object_id=away_team.id
            ).first()
    ...
    def get_data(self, **kwargs):
        ...
        event_slug = kwargs['slug']
        event = Events.objects.get(slug=event_slug)
        data = {'home_score': event.home_score,'away_score': event.home_score,}  # Замените на фактические данные
