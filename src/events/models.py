        ...
        while not self.slug and not unique:
            if loop_num < settings.RANDOM_URL_MAX_TRIES:
                ...
                for i in range(settings.RANDOM_URL_LENGTH):
                    new_key += settings.RANDOM_URL_CHARSET[randrange(0, len(settings.RANDOM_URL_CHARSET))]
...
class Incidents(models.Model):
    ...
    incident_participants = models.ManyToManyField(
        "IncidentParticipants", verbose_name="Участники инцидента", blank=True
    )
